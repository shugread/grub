...
#endif
#ifdef __i386__
/* GCC将前三个参数放入寄存器（依次为EAX、EDX、ECX），剩余的参数才通过栈传递。 */
#define REED_SOLOMON_ATTRIBUTE __attribute__((regparm(3)))
#else
#define REED_SOLOMON_ATTRIBUTE
#endif
void grub_reed_solomon_recover(void *ptr_, grub_size_t s,
                               grub_size_t rs) REED_SOLOMON_ATTRIBUTE;
#else
#define REED_SOLOMON_ATTRIBUTE
#endif
...
static gf_single_t gf_mul(gf_single_t a, gf_single_t b) {
  if (a == 0 || b == 0)
    return 0;
  return gf_powx[(int)gf_powx_inv[a] + (int)gf_powx_inv[b]];
}

static inline gf_single_t gf_invert(gf_single_t a) {
  return gf_powx[255 - (int)gf_powx_inv[a]];
}

static void init_powx(void) {
  int i;
  grub_uint8_t cur = 1;
...
static gf_single_t pol_evaluate(gf_single_t *pol, grub_size_t degree,
                                int log_x) {
  int i;
  gf_single_t s = 0;
  int log_xn = 0;
...
static void rs_encode(gf_single_t *data, grub_size_t s, grub_size_t rs) {
...
static void gauss_eliminate(gf_single_t *eq, int n, int m, int *chosen) {
  int i, j;
  ...
  for (i = 0; i < n; i++) {
    ...
    int k;
    gf_single_t r;
    for (nzidx = 0; nzidx < m && (eq[i * (m + 1) + nzidx] == 0); nzidx++)
      ;
    if (nzidx == m)
      continue;
    chosen[i] = nzidx;
...
static void gauss_solve(gf_single_t *eq, int n, int m, gf_single_t *sol) {
  int i, j;
...
static void rs_recover(gf_single_t *mm, grub_size_t s, grub_size_t rs) {
    ...
    }
  {
    ...
    for (i = 1; i < (int)rs; i++) {
      for (j = 0; j < (int)errnum; j++)
        eqstat[(errnum + 1) * i + j] =
            gf_mul(errpot[j], eqstat[(errnum + 1) * (i - 1) + j]);
...
static void decode_block(gf_single_t *ptr, grub_size_t s, gf_single_t *rptr,
                         grub_size_t rs) {
  int i, j;
...
#if !defined(STANDALONE)
static void encode_block(gf_single_t *ptr, grub_size_t s, gf_single_t *rptr,
                         grub_size_t rs) {
  int i, j;
  for (i = 0; i < SECTOR_SIZE; i++) {
    grub_size_t ds = (s + SECTOR_SIZE - 1 - i) / SECTOR_SIZE;
...
#if !defined(STANDALONE)
void grub_reed_solomon_add_redundancy(void *buffer, grub_size_t data_size,
...
/**
 * 实现 Reed-Solomon 纠错算法的函数, 在纠错过程中恢复丢失或错误的数据块
 * @param prt_ 指向数据的指针
 * @param s 原始数据块的大小
 * @param rs 冗余数据块的大小，表示纠错码（冗余信息）的长度
 */
void REED_SOLOMON_ATTRIBUTE grub_reed_solomon_recover(void *ptr_, grub_size_t s,
                                                      grub_size_t rs) {
  gf_single_t *ptr = ptr_;
  gf_single_t *rptr = ptr + s; // 指向冗余数据的起始位置
  grub_uint8_t *cptr;
  ...
  for (cptr = rptr + rs - 1; cptr >= rptr; cptr--)
    if (*cptr)
      break;
  // 如果冗余数据部分中的有效数据（非零数据）和冗余数据的距离超过冗余块的一半，说明冗余部分的质量较差，无法有效进行恢复，函数返回。
  if (rptr + rs - 1 - cptr > (grub_ssize_t)rs / 2)
    return;

  // 初始化 Reed-Solomon 算法中需要的幂次表
  init_powx();

  while (s > 0) {
    grub_size_t tt;
    grub_size_t cs, crs;
    // cs 和 crs 分别代表当前数据块和冗余块的大小
    cs = s;
    crs = rs;
    tt = cs + crs;
    // 如果总和超过最大块大小（MAX_BLOCK_SIZE），则按比例调整 cs 和 crs
    if (tt > MAX_BLOCK_SIZE) {
      cs = ((cs * (MAX_BLOCK_SIZE / 512)) / tt) * 512;
      crs = ((crs * (MAX_BLOCK_SIZE / 512)) / tt) * 512;
    }
